...
const propTypes = {
  ...
  /**
   * Callback when route loaded.
   */
  onRouteLoaded: PropTypes.func,
};
...
class MapRoute extends React.Component {
  ...
  constructor(props) {
    super(props);
    this._onRouteLoaded = this._onRouteLoaded.bind(this);
  }
  ...
  _onRouteLoaded(e) {
    if (this.props.onRouteLoaded) {
      this.props.onRouteLoaded(e.nativeEvent);
    }
  }
  ...
  convertProps(props) {
    return {
      ...
      onRouteLoaded: this._onRouteLoaded,
    };
  }

  render() {
    const AIRMapRoute = this.getAirComponent();

    return (
      <AIRMapRoute {...this.convertProps(this.props)} ref={ref => { this.polyline = ref; }} />
    );
  }
}

MapRoute.propTypes = propTypes;
MapRoute.defaultProps = defaultProps;

module.exports = decorateMapComponent(MapRoute, {
  componentType: 'Route',
  providers: {
    google: {
      ios: NOT_SUPPORTED,
